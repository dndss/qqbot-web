...
SDK 按 `msg_id` 在内存中维护 5 分钟的回复序号。遇到腾讯接口返回
`40054005`（消息被去重）时，会把 `msg_seq` 依次递增，最多尝试 `1～5`。

如果接口返回 `40034005`（`msg_id` 已过期），SDK 会移除 `msg_id` 和
`msg_seq`，自动改用主动消息发送。

引用回复仍会优先使用被动回复结构。如果被动发送最终失败，SDK 会保留
`message_reference`，移除 `msg_id` 和 `msg_seq` 后作为主动引用消息重试一次：
...
因此引用显示不会因为被动回复失败而丢失；主动重试仍失败时抛出主动发送的错误。
